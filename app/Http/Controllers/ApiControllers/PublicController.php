...
class PublicController extends Controller
{
    ...
    public function statistics()
    {
        return Cache::remember('public_stats_aggregation_v2', 300, function () {
            ...
            // 6. Recent Activities
            $recentZakat = DB::table('muzaki')->latest()->limit(5)->get()->map(fn($z) => [
                'nama' => $this->maskName($z->nama), 'tipe' => 'Zakat Fitrah', 'nominal' => $z->jumlah_beras_kg . ' KG', 'tanggal' => $z->created_at
            ]);
            $recentZakatMal = DB::table('zakat_malls')->latest('tanggal')->limit(5)->get()->map(fn($zm) => [
                'nama' => $this->maskName($zm->nama_muzaki), 'tipe' => 'Zakat Mal', 'nominal' => (float) $zm->jumlah, 'tanggal' => $zm->tanggal
            ]);
            $recentSedekah = DB::table('sedekah')->latest('tanggal')->where('jenis', 'penerimaan')->limit(5)->get()->map(fn($s) => [
